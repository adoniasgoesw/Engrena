...
const TabelaRelatorioOrdens = ({ dados, onDetalhes, onImprimir }) => {
  if (dados.length === 0) {
    return (
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white border border-gray-200 rounded-lg">
        ...
        <tbody className="divide-y divide-gray-200">
          {dados.map((ordem) => {
            const dataFinalizacao = ordem.data_fechamento || ordem.criado_em
            return (
              <tr key={ordem.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                  {formatDate(dataFinalizacao)}
                </td>
...
export default TabelaRelatorioOrdens
